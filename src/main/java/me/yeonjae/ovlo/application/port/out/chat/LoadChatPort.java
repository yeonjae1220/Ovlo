...
public interface LoadChatPort {
    ...
    Map<Long, Instant> findAllLastReadAt(ChatRoomId chatRoomId);

    /** 여러 채팅방의 읽음 마커 일괄 조회 (chatRoomId → (memberId → lastReadAt)) */
    Map<Long, Map<Long, Instant>> findAllLastReadAtByRoomIds(List<ChatRoomId> roomIds);

    /** 특정 회원의 마지막 읽음 시각 */
    Optional<Instant> findLastReadAt(ChatRoomId chatRoomId, MemberId memberId);

    /** since 이후에 senderId가 아닌 사람이 보낸 메시지 수 */
    long countUnread(ChatRoomId chatRoomId, MemberId memberId, Instant since);

    /** 여러 채팅방의 안 읽은 메시지 수 일괄 조회 (chatRoomId → unreadCount) */
    Map<Long, Long> countUnreadBatch(MemberId memberId, Map<Long, Instant> sinceByRoomId);
