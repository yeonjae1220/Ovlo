...
class CurrencyCleanupTest(unittest.TestCase):
 def test_foreign_conversions(self):
  for raw,expected,local in [
   ...
   ('€600 (约6万韩元)','€600',['EUR']),
   ('¥800（约合7美元）','¥800',['JPY']),
   ('€120 (tương đương 60.000 won)','€120',['EUR']),
   ...
   ('¥200k-400k ($1.4k-$2.8k USD)','¥200k-400k',['JPY']),
 ...
 def test_scale_tokens_do_not_consume_following_words(self):
  ...
  self.assertEqual('¥40–60 million',monies('¥40–60 million之间',['JPY'])[0].text)
  self.assertEqual('¥100,000',monies('¥100,000 1人用',['JPY'])[0].text)
 def row(self):
