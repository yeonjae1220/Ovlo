...
export default function Error({
  ...
  return (
    ...
    >
      <p style={{ fontSize: 15, color: 'var(--color-text, inherit)' }}>{t('error.unexpected')}</p>

      {/* digest 는 서버 로그와 이 화면을 잇는 유일한 상관관계 키다. */}
      {error.digest && <code style={{ fontSize: 11, color: 'var(--color-text-muted, #888)' }}>{error.digest}</code>}
