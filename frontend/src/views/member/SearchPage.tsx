...
export default function SearchPage() {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      {isFetching && <p style={{ color: '#888', fontSize: 14 }}>{t('search.searching')}</p>}

      <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
        {results
          ?.filter((m) => String(m.id) !== String(currentUser?.id))
          .map((m) => (
            <li key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid #eee' }}>
              <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <Link href={`/profile/${m.id}`} style={{ fontWeight: 600, textDecoration: 'none', color: '#000', display: 'block' }}>
                  {m.nickname}
                </Link>
      ...
      {results?.filter((m) => String(m.id) !== String(currentUser?.id)).length === 0
        && query.length >= 1
        && !isFetching && (
          <p style={{ color: '#888', fontSize: 14 }}>{t('search.noResults')}</p>
        )}
    </div>
  )
}
